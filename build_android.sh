...
rm -rf native/build/android/* 2>/dev/null || true
rm -rf native/android/src/main/jniLibs/* 2>/dev/null || true
rm -rf native/target 2>/dev/null || true


# ==============================
# 2. Install Native targets if needed
# ==============================
echo -e "\n${YELLOW}🔧 Checking/installing Native targets...${NC}"
...
if [ ! -f "Cargo.toml" ]; then
    echo -e "${RED}❌ Error: Cargo.toml not found in native/${NC}"
    exit 1
fi
